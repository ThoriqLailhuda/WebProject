<form action="{{url('save_bhp')}}" method="post" required>
                                {{csrf_field()}}
                                Nama 
                                <select name="id_bhp" class="form-control" id="id_obat1" onchange="biaya_tindakan('id_obat1','harga1')">
                                <option selected disabled> --  Pilih Tindakan --  </option>
                                    <?php foreach($ref_bhp as $input) { ?>
